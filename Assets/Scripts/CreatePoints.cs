...
public class CreatePoints : MonoBehaviour {
	...
	/*
	 * not used for the meantime
	public void Undo() {
		//erase last step
		ballsCount--;
		GameObject[] g = GameObject.FindGameObjectsWithTag ("Prefab");
		Destroy (g [g.Length - 1]);
		Destroy (g[g.Length - 2]);
		Destroy (g [g.Length - 3]);
		distance [ballsCount -2] = 0;
		string s;
		if (ballsCount <= 2) {
			Reset ();
		}
		s = System.String.Format ("{0:0.00}", distance [ballsCount - 3] * toInches);
		distText.text = "Distance:                       " + s + " inches ";
	}
	*/
